...
class LaneSubscriber : public rclcpp::Node
{
    ...
    LaneSubscriber() : Node("lane_subscriber")
        {
            ...
            this->declare_parameter("linear_velocity", 5.0);
            stop = 0; //variable for start and stop
            ...
            ctrl_publisher_ = this->create_publisher<geometry_msgs::msg::TwistStamped>("/car1/cmd_vel",10);

            // subscriber aruco code
            //aruco_sub_ = this->create_subscription<std_msgs::msg::Int32>("/car1/nav_cmd", 10, std::bind(&LaneSubscriber::aruco_callback, this, std::placeholders::_1));
        }
    ...
    void aruco_callback(const std_msgs::msg::Int32::SharedPtr msg)
    {
        ...
        if (aruco == 16)
        {
            ...
            stop = 1; // Stop the car until a new aruco is detected
        }
        else if (aruco == 42)
        {
            cout << "start detected" << endl;
            stop = 0; // Aruco restart detected, now the car can move
        }
    }
    ...
    private:
        ...
        {   
            ...
            if (stop == 1) { // Controlla se il codice "stop" di Aruco è stato rilevato
                twist_msg.twist.linear.x = 0.0;
            } else {
                twist_msg.twist.linear.x = control_result[0];
            }
        ...
        int stop=0; //global variable stop for aruco stop&go
        bool is_on_ = false;
